...
joint.ui.Stencil = Backbone.View.extend({
    ...
    render: function () {
        ...
        if (this.options.groups) {
            ...
            _.each(sortedGroups, function (groupArray) {
                ...
                var $group = $(joint.templates.stencil["group.html"]({
    ...
    drop: function (evt, cell, cellViewBBox) {
        ...
        if (paperArea.containsPoint(p)) {
            ...
            cell.unset("z");
            if(cell.attr().attributes.type=="iot.LibDevice"){
					if(libdevicesLoaded[$("#libname").val()]!= undefined && libdevicesLoaded[$("#libname").val()]!=""){
						var layot = JSON.parse(libdevicesLoaded[$("#libname").val()]);
                    	app.graph.addCells(layot.layout.cells);
					}
			}
			else{
				graph.addCell(cell);
				addIndexAndGroup(cell,p);
				reparentEverything(cell,p);
			}
	    }
    },
